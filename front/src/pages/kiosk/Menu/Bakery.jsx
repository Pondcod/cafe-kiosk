...
const Bakery = () => {
  ...
  // 1) Filter items whose subCategory includes "Bakery"
  const bakeryMenus = Menu.filter(
    item =>
      Array.isArray(item.subCategory) &&
      item.subCategory.includes('Bakery')
  );
    ...
    >
      ...
      <div className="flex flex-col flex-1 h-screen overflow-hidden">
        <div className="flex-1 flex flex-col p-6 overflow-y-auto mr-10">
          <h1 className="text-5xl font-medium mb-6">Bakery</h1>
          <div className="grid grid-cols-3 gap-y-10 gap-x-6 place-items-center">
            {bakeryMenus.map((item, idx) => (
              <div
                key={idx}
      ...
      {isCustomizationOpen && selectedItem && (
        <CustomizationSlide
          hasMilk={false} // bakery items have no milk option
          initialCustomization={customizationDetails}
