...
const Milk = () => {
  ...
  // 1) Filter all items whose subCategory includes "Milk"
  const milkMenus = Menu.filter(
    item =>
      Array.isArray(item.subCategory) &&
      item.subCategory.includes('Milk')
  );

  // 2) Unique by mainCategory
  const uniqueMap = {};
  milkMenus.forEach(item => {
    ...
    >
      ...
      <div className="flex flex-col flex-1 h-screen overflow-hidden">
        <div className="flex-1 flex flex-col p-6 overflow-y-auto mr-10">
          <h1 className="text-5xl font-medium mb-6">Milk</h1>
