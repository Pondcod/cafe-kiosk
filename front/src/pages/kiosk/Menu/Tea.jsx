...
const Tea = () => {
  ...
  // 1) Filter all items whose subCategory includes "Tea"
  const teaMenus = Menu.filter(
    item =>
      Array.isArray(item.subCategory) &&
      item.subCategory.includes('Tea')
  );

  // 2) Unique by mainCategory
  const uniqueMap = {};
  teaMenus.forEach(item => {
    ...
    >
      ...
      <div className="flex flex-col flex-1 h-screen overflow-hidden">
        <div className="flex-1 flex flex-col p-6 overflow-y-auto mr-10">
          <h1 className="text-5xl font-medium mb-6">Tea</h1>
