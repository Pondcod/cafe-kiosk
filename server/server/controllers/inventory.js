...
const createInventory = async (req, res) => {
  try {
    ...
    console.log('Creating inventory item with data:', { item_name, quantity, unit, reorder_level, product_id });
...
const updateInventory = async (req, res) => {
  try {
    const { id } = req.params;
    const { item_name, quantity, unit, reorder_level, product_id } = req.body;
    console.log(`Updating inventory ${id} with data:`, { item_name, quantity, unit, reorder_level, product_id });
    ...
    if (product_id !== undefined) {
      ...
      if (product_id !== null && (!productData || productData.length === 0)) {
        return res.status(400).json({ 
          success: false, 
          message: `Product with ID ${product_id} not found` 
        });
...
const getLowStockInventory = async (req, res) => {
  try {
    ...
    // SQL query to get items where quantity is below reorder level
    const { data, error } = await supabase
      .from('inventory')
      .select(`
        *,
    ...
    console.log(`Retrieved ${data ? data.length : 0} low stock inventory items`);
    res.json({ success: true, data });
  } catch (error) {
